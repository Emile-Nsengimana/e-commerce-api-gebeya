import { itemSchema, updateItemSchema } from "./schema/item";
...
class ItemValidation {
    ...
    try {
      ...
      if (req.user.role !== "vendor")
        return res.status(403).json({
          error: "you are not authorized to add an item",
        });
  ...
  static async updateItemValidator(req, res, next) {
    const { name, status, quantity, price, description } = req.body;
    const item = { name, status, quantity, price, description };
    try {
      const checkItem = updateItemSchema.validate(item, { abortEarly: false });
      const errors = [];
